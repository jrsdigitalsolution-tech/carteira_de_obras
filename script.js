const ITENS = ["BBA/ELET.", "MT", "FLUT.", "M FV.", "AD. FLEX", "AD. RIG.", "FIXADORES", "SIST. ELÉT.", "PEÇAS REP.", "SERV.", "MONT.", "FATUR."];
  ...
  function notify(m) {
    ...
    t.innerHTML = m; // changed to innerHTML to allow emojis and formatting
    area.appendChild(t);
    setTimeout(() => t.remove(), 4000); // 4 seconds
  }
  ...
  function renderizar(dadosOriginais) {
    ...
    if (!isGeralView) {
      ...
      const labs = ["ABERTURA", "OBRA", "CLIENTE", "STATUS", "ITEM", "CATEG. / SEGMENTO", "RESPONSÁVEL", "COMPLEX.", "UF", "ETAPA", "PRAZO", "NF", "PREÇO", "% ORÇADO"];
      ...
      dadosOrdenados.forEach(dO => {
        ...
        const statusCor = r[COLS.STATUS_PROPOSTA] === 'FRUSTRADAS' ? 'days-urgent' : 'days-ok';
        ...
        html += `<td><span class="days-badge ${statusCor} shadow-sm">${r[COLS.STATUS_PROPOSTA] || "-"}</span></td>`;
        html += `<td class="td-read-left"><div class="text-truncate" style="max-width:150px" title="${r[COLS.ITEM_GERAL]}">${r[COLS.ITEM_GERAL] || "-"}</div></td>`;
  ...
  function abrirModalCompra(id, mode = 'DATA') {
    ...
    if (inputValor) inputValor.value = document.getElementById(`${id}_valor_val`).value;
  ...
  function abrirNovo() { 
    // Mantida para chamadas via código, mesmo sem botão físico
  ...
  function salvar() {
    ...
    try {
      ...
      const obj = { 
        ...
        cliente: document.getElementById('cliente').value.trim() || "", valor: sanitizeMoneyText(document.getElementById('valor').value), 
  ...
  function editar(idx) {
    ...
    document.getElementById('data_entrada_orig').value = r[COLS.DATA] || ""; document.getElementById('obra').value = r[COLS.OBRA] || "";
    document.getElementById('cliente').value = r[COLS.CLIENTE] || ""; document.getElementById('valor').value = r[COLS.VALOR] || "";
    document.getElementById('dias_prazo').value = r[COLS.DIAS_PRAZO] || ""; document.getElementById('analise').value = r[COLS.OBS] || "";
    document.getElementById('cpmv_obra_val').value = r[COLS.CPMV] || 0;
    ...
    ITENS.forEach((it, i) => {
      ...
      if (det[id] && typeof det[id] === "object") {
        ...
        if (document.getElementById(id + '_valor_val')) document.getElementById(id + '_valor_val').value = det[id].preco || "";
  ...
  function abrirResumoGeral() {
    ...
    callServer('getResumoGeralObra', [obra], res => {
      ...
      const toNumber = (v) => { const s = String(v ?? "").replace(/\./g, "").replace(",", ".").replace(/[^\d.-]/g, ""); const n = Number(s); return Number.isFinite(n) ? n : 0; };

      const total = toNumber(mapa["P. TOTAL"]); const recebido = toNumber(mapa["RECEB."]); const carteira = toNumber(mapa["A RECEB"]); const percentualRecebido = total > 0 ? Math.min((recebido / total) * 100, 100) : 0;
